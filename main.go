...
func main() {
	var mode = flag.String("mode", "random", "Visualization mode: matrix, nyancat, snake, missiledefender, spectrograph, snowflakes, waterripple, lightning, heatmap, or random")
	...
	allModes := []string{"matrix", "nyancat", "snake", "missiledefender", "spectrograph", "snowflakes", "waterripple", "lightning", "heatmap"}
	...
	for {
		...
		case "heatmap":
			cycleToNext = runHeatmap(screen, sigChan, *interactive, *grayscale)
		default:
			screen.Fini()
			fmt.Fprintf(os.Stderr, "Unknown mode: %s. Use: matrix, nyancat, snake, missiledefender, spectrograph, snowflakes, waterripple, lightning, heatmap, or random\n", *mode)
			os.Exit(1)
		}
